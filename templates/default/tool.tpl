...
{foreach $services item}
            ...
            <tr>
                ...
                <td>{if $item.follow_url && user_has_perm('socialmedia_edit')}
                    <label class="form-check-label custom-control custom-checkbox">
                        <input data-item="{$item.id}" data-url="follow" name="socialmedia_follow_enabled_{$item.id}" id="socialmedia_follow_enabled_{$item.id}" class="form-check-input custom-control-input" type="checkbox" value="Y"{if ! $item.follow_disabled=="Y"} checked="checked"{/if} />
            ...
            <tr>
                ...
                <td>{if $item.share_url && user_has_perm('socialmedia_edit')}
                    <label class="form-check-label custom-control custom-checkbox">
                        <input data-item="{$item.id}" data-url="share" name="socialmedia_share_enabled_{$item.id}" id="socialmedia_share_enabled_{$item.id}" class="form-check-input custom-control-input" type="checkbox" value="Y"{if ! $item.share_disabled=="Y"} checked="checked"{/if} />
                        <span class="custom-control-indicator" title="{translate('Enable/disable for whole site')}"></span>
                    </label>{else}<span style="display:inline-block;width:4.2em;"></span>{/if}
                </td>
                <td>
